...
| --- | --- | --- |
| `GOOGLE/DYNAMICWORLD/V1` | Recent water observation for current visible spread | Optical, 10 m, 45-day lookback window |
...
2. Use the latest available image date in that window as `observation_end` and `summary_date`.
3. Compute the mean `water` band across the whole window.
4. Multiply that mean water signal by pixel area.
5. Sum the result inside the polygon and convert to hectares.

Important interpretation note:

- this is an estimated visible surface-water area, not a field-survey shoreline
- because the method uses a mean probability-like water signal, it should be read as a smoothed operational estimate rather than a strict binary water mask
...
The app uses the same calendar month as the current observation month. That means April observations are compared to the historical April wetness pattern, not to a year-round average.
...
This is what powers sentences like:

- `Historically holds water in about 100% of months.`
