...
- Repo metadata already includes catchment area for these four in [src/lib/utils/constants.ts](src/lib/utils/constants.ts).
...
- the repo already uses GitHub Actions for scheduled data work in [.github/workflows/daily-data-pipeline.yml](.github/workflows/daily-data-pipeline.yml)
...
- [public/geojson/chennai-water-bodies-current.geojson](public/geojson/chennai-water-bodies-current.geojson)
- [public/data/restoration-priority.json](public/data/restoration-priority.json)
...
- keep [src/components/dashboard/reservoir-cards.tsx](src/components/dashboard/reservoir-cards.tsx) mostly unchanged
- surface catchment context in the selected-reservoir drilldown area inside [src/components/dashboard/dashboard-content.tsx](src/components/dashboard/dashboard-content.tsx) or the chart header section
...
Add one compact "Satellite context" section in [src/components/water-bodies/unified-detail-panel.tsx](src/components/water-bodies/unified-detail-panel.tsx).
...
This can be added in [src/components/insights/city-story.tsx](src/components/insights/city-story.tsx) after the core data path is stable.

## API plan

### Reservoir data path

Read directly from Supabase in [src/app/page.tsx](src/app/page.tsx) because the page already does server-side Supabase reads.
...
- add `earthengine-api` to [neer-vazhvu-api/pyproject.toml](neer-vazhvu-api/pyproject.toml)
