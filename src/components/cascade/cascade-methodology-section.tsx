...
const PARAMETER_LABELS: Record<string, string> = {
  max_downstream_distance_km: "max_downstream_distance_km",
  cone_halfangle_deg: "cone half-angle (degrees)",
  min_tank_area_ha: "min_tank_area_ha",
  max_river_outlet_distance_km: "max_river_outlet_distance_km",
};

const PARAMETER_RATIONALE: Record<string, string> = {
...
export function CascadeMethodologySection({
  ...
  sensitivity?: CascadeSensitivity | null;
}): ReactNode {
  return (
    <div className="space-y-4 text-sm text-slate-700 dark:text-slate-300">
      ...
      {sensitivity ? (
        <div>
          ...
          <p className="mb-3">
            Each tunable parameter was chosen with a stated rationale.
            The sensitivity tables below show how each output statistic
            responds when the parameter is varied. Generated by the
            cascade pipeline&apos;s{" "}
            <code className="text-xs">sensitivity</code> stage; raw data
            at{" "}
            <code className="text-xs">
              public/data/cascade/{cityId ?? sensitivity.district_id}-cascade-sensitivity.json
            </code>
            .
          </p>
          <div className="space-y-5">
            {sensitivity.sweeps.map((sweep) => (
              <ParameterSensitivityBlock key={sweep.parameter} sweep={sweep} />
            ))}
          </div>
        </div>
      ) : null}
...
function ParameterSensitivityBlock({
  sweep,
}: {
  sweep: import("@/lib/cascade-sensitivity").SensitivitySweep;
}) {
  const label = PARAMETER_LABELS[sweep.parameter] ?? sweep.parameter;
  const rationale = PARAMETER_RATIONALE[sweep.parameter];
  return (
    <div className="border-l-2 border-slate-200 dark:border-slate-700 pl-4">
      <div className="mb-1">
        <code className="text-xs font-semibold text-slate-800 dark:text-slate-200">
          {label}
        </code>{" "}
        <span className="text-xs text-slate-500 dark:text-slate-400">
          default <strong>{sweep.default}</strong>
        </span>
      </div>
      {rationale ? <p className="mb-2 text-sm">{rationale}</p> : null}
      <div className="overflow-x-auto">
        <table className="text-xs min-w-full">
          <thead>
            <tr className="text-slate-500 dark:text-slate-400 text-left">
              <th className="pr-3 py-1 font-medium">value</th>
              <th className="pr-3 py-1 font-medium text-right">nodes</th>
              <th className="pr-3 py-1 font-medium text-right">edges</th>
              <th className="pr-3 py-1 font-medium text-right">isolated</th>
              <th className="pr-3 py-1 font-medium text-right">max depth</th>
              <th className="pr-3 py-1 font-medium text-right">outlets</th>
            </tr>
          </thead>
          <tbody>
            {sweep.results.map((r) => {
              const isDefault = r.value === sweep.default;
              const rowClass = isDefault
                ? "bg-sky-50 dark:bg-sky-950/40 font-medium text-slate-900 dark:text-slate-100"
                : "text-slate-600 dark:text-slate-400";
              return (
                <tr key={r.value} className={rowClass}>
                  <td className="pr-3 py-1 tabular-nums">{r.value}</td>
                  {r.skipped ? (
                    <td colSpan={5} className="py-1 italic text-slate-500">
                      skipped: {r.note}
                    </td>
                  ) : (
                    <>
