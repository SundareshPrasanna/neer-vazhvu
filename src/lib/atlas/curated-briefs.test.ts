...
import { curatedBriefCount, getCuratedBrief, getCuratedBriefs } from "./curated-briefs";
import { getDistrictBrief } from "./district-directory";
...
test("the reviewed Thanjavur briefs resolve by LGD code", () => {
  assert.equal(curatedBriefCount("thanjavur"), 4);
  for (const lgdCode of CURATED_LGD_CODES) {
    const curated = getCuratedBrief("thanjavur", lgdCode);
...
test("a reviewed brief says something the generated one cannot", () => {
  const curated = getCuratedBrief("thanjavur", "228711");
  const generated = getDistrictBrief("thanjavur", "228711");
