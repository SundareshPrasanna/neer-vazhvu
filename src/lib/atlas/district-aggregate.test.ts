...
test("every Panchayat is counted once, brief or not", () => {
  for (const fixture of FIXTURE_DISTRICTS) {
    ...
    const directory = buildFixtureDirectory(fixture.slug);
...
test("a measure reports the places that carried it, not the whole district", () => {
  const aggregate = buildFixtureAggregate("thanjavur", AS_OF);
...
test("staleness is counted against the as-of date, not today", () => {
  const early = buildFixtureAggregate("tiruchirappalli", "2026-01-01");
  const later = buildFixtureAggregate("tiruchirappalli", "2026-07-26");
  assert.ok(later.stalePlaces >= early.stalePlaces);
  assert.ok(later.worstSampleAgeDays !== null);
  const thanjavur = buildFixtureAggregate("thanjavur", AS_OF);
...
test("no place is double counted between blocks", () => {
  const directory = buildFixtureDirectory("thanjavur");
  const briefs = new Set(fixtureBriefs("thanjavur").map((b) => b.placeId));
