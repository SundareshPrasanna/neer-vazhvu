...
export const KABINI: BasinManifest = {
  ...
  layers: [
    ...
    // The Kerala tributary skeleton: every named watercourse drawn whole
    // (plus unnamed trunk pieces where naming lapses), so the two context
    // reservoirs sit on their rivers and no river surfaces mid-course
    // (Sundaresh, 01 Sep, both rounds).
