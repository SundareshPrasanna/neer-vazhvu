...
Per-city *features* live alongside in the same folder: [docs/cities/chennai/features.md](docs/cities/chennai/features.md), [docs/cities/madurai/features.md](docs/cities/madurai/features.md), [docs/cities/bangalore/features.md](docs/cities/bangalore/features.md), [docs/cities/mumbai/features.md](docs/cities/mumbai/features.md), [docs/cities/delhi/features.md](docs/cities/delhi/features.md), [docs/cities/kolkata/features.md](docs/cities/kolkata/features.md), and [docs/cities/surat/features.md](docs/cities/surat/features.md). Kolkata and Surat additionally ship a graded parity scorecard against Chennai at [docs/cities/kolkata/parity-scorecard.md](docs/cities/kolkata/parity-scorecard.md) and [docs/cities/surat/parity-scorecard.md](docs/cities/surat/parity-scorecard.md) - every feature scored XHigh/High/Medium/Low/N-A with the reason recorded wherever parity is not reachable.

> **This index is itself incomplete, which is worth saying rather than hiding.** Hyderabad and Gurugram are live and have no entry above; Hyderabad ships `docs/cities/hyderabad/data-sources.md` and neither ships a features document. The registry in `src/lib/cities/` is the authoritative roster - this list is prose that has to be maintained by hand, and it has fallen behind it three times.

When adding a city, copy the Surat, Kolkata or Delhi folder as a template - those docs reflect the multi-city naming convention (per-city `-<cityId>` suffix on data files). Chennai's docs predate that and use unsuffixed legacy paths for back-compat.
