/**
 * Fetch CGWB groundwater resource assessment data from India WRIS ArcGIS REST API.
 *
 * Fetches:
 * 1. Block-level GWR data for Chennai across all available years (2011-2024)
 * 2. Block boundary polygons (GeoJSON) from GWR2024
 * 3. CGWB groundwater monitoring station locations
 *
 * Output:
 * - public/data/gwr-blocks.json         (block data + multi-year history)
 * - public/geojson/chennai-gwr-blocks.geojson  (block boundary polygons)
 * - public/data/gw-stations.json        (monitoring well locations)
 *
 * Run: npx tsx scripts/fetch-wris-groundwater.ts
 */
...
interface GWRRecord {
  block: string;
  class: string; // Safe, Semi Critical, Over Exploited, Critical
  sgw_dev_pe: number; // Stage of GW development %
  na_gwa: number | null; // Net annual GW availability (HAM)
  agwd_tot: number | null; // Annual GW draft total (HAM)
  agwd_irr: number | null; // Annual GW draft irrigation
  agwd_dom_i: number | null; // Annual GW draft domestic+industrial
}

interface GWRYearData {
  year: number;
  records: GWRRecord[];
}
...
function normalizeBlockName(raw: string): string {
  ...
  const mappings: Record<string, string> = {
    "mambalam - guindy": "Mambalam",
    "mambalam -guindy": "Mambalam",
    "mambalam": "Mambalam",
    "egmore - nungambakkam": "Egmore",
    "egmore - nungambakka": "Egmore",
    "egmore-nungambakkam": "Egmore",
    "mylapore - tiruvallikeni": "Mylapore",
    "mylapore - tiruvalli": "Mylapore",
    "purasawalkam - perambur": "Purasaivakkam",
    "purasawalkam-perambur": "Purasaivakkam",
    "purasawalkam -  pera": "Purasaivakkam",
    "purasawalkam -  perambur": "Purasaivakkam",
    "purasawalkam -pera": "Purasaivakkam",
    "purasaivakkam": "Purasaivakkam",
    "perambur": "Perambur",
    "kottai - thondiarpet": "Tondiarpet",
    "alanthur": "Alandur",
    "pallikaranai": "Sholinganallur", // merged into Sholinganallur block
    "korattur": "Ambattur", // merged
    "manali": "Thiruvottiyur", // merged
    "porur": "Maduravoyal", // merged
    "maduravoil": "Maduravoyal",
    "velacherry": "Velacheri",
  };

  // Strip roman numeral suffix (e.g., "-I", "-II", "-III", "-IV", "-IIi", "-Iv")
  const stripped = name.replace(/[-\s]+[IiVv]+\s*$/i, "").trim();
  const key = stripped.toLowerCase();

  if (mappings[key]) return mappings[key];

  // Title case the name
  return name.replace(/[-\s]+[IiVv]+\s*$/i, "").trim()
    .split(/\s+/)
    .map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
    .join(" ");
}

async function fetchGWRYear(year: number): Promise<GWRYearData> {
  const url = `${WRIS_BASE}/GWR${year}_CGWB/MapServer/8/query?` +
    `where=state%3D%27TN%27+AND+district+LIKE+%27%25hennai%25%27` +
  ...
  const records: GWRRecord[] = (data.features || []).map((f: { attributes: Record<string, unknown> }) => ({
...
async function fetchBlockBoundaries(): Promise<GeoJSON.FeatureCollection> {
  const url = `${WRIS_BASE}/GWR2024_CGWB/MapServer/8/query?` +
    `where=state%3D%27TN%27+AND+district+LIKE+%27%25hennai%25%27` +
...
async function fetchStations(): Promise<{ name: string; lat: number; lng: number; agency: string; block: string; station_code: string; data_types: string }[]> {
  const url = `${WRIS_BASE}/GroundwaterLevel_Stations/MapServer/0/query?` +
    `where=district_name+LIKE+%27%25Chennai%25%27` +
  ...
  const stations = (data.features || []).map((f: { attributes: Record<string, unknown> }) => ({
  ...
  // Deduplicate by name + rounding coords to 3 decimal places
...
async function main() {
  console.log("Fetching WRIS groundwater data for Chennai...\n");

  // 1. Fetch GWR data for all years
  console.log("1. Fetching GWR block assessments:");
  const yearData: GWRYearData[] = [];
  ...
  // 2. Normalize and aggregate by block
  console.log("\n2. Normalizing block names and aggregating history:");
  const blockMap = new Map<string, NormalizedBlock>();

  for (const yd of yearData) {
    // Aggregate sub-blocks: sum availability and draft, average development %
    const blockAgg = new Map<string, { classes: string[]; devs: number[]; avails: number[]; drafts: number[] }>();
    ...
    for (const [name, agg] of blockAgg) {
      if (!blockMap.has(name)) {
        blockMap.set(name, { name, history: [], latest: { class: "", development_pct: 0, availability_ham: null, draft_total_ham: null } });
      }
      const block = blockMap.get(name)!;
      // Use worst class among sub-blocks
      const classOrder = ["Safe", "Semi Critical", "Semi-Critical", "Critical", "Over Exploited"];
      const worstClass = agg.classes.sort((a, b) => classOrder.indexOf(b) - classOrder.indexOf(a))[0];
      // Weighted average dev % (by availability)
      const totalAvail = agg.avails.reduce((s, v) => s + v, 0);
      const totalDraft = agg.drafts.reduce((s, v) => s + v, 0);
      const avgDev = totalAvail > 0 ? (totalDraft / totalAvail) * 100 : agg.devs.reduce((s, v) => s + v, 0) / agg.devs.length;
  ...
  // Set latest from most recent year
  ...
  const blocks = [...blockMap.values()].sort((a, b) => b.latest.development_pct - a.latest.development_pct);
  console.log(`  ${blocks.length} normalized blocks`);
  for (const b of blocks) {
    console.log(`    ${b.name.padEnd(20)} ${b.latest.class.padEnd(18)} dev=${b.latest.development_pct}%`);
  }

  // 3. Fetch block boundaries
  console.log("\n3. Fetching block boundary polygons:");
  const boundaries = await fetchBlockBoundaries();

  // 4. Fetch monitoring stations
