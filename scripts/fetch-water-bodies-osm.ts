/**
 * One-time script: Fetch Chennai water bodies from OpenStreetMap via Overpass API.
 * Saves GeoJSON to public/geojson/chennai-water-bodies-current.geojson
 *
...
const BBOX = "12.7,79.9,13.4,80.4";
...
function computePolygonAreaHa(coords: number[][]): number {
  ...
  // 1 degree² ≈ 111320² m² at Chennai latitude (13°N)
  const m2 = area * 111320 * 111320 * Math.cos((13 * Math.PI) / 180);
  return Math.round((m2 / 10000) * 100) / 100;
}

async function main() {
  console.log("Querying Overpass API for Chennai water bodies...");
