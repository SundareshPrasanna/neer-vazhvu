#!/usr/bin/env python3
"""Stamp NVDM v1 envelopes on the krishnagiri-rivers basin artifacts (L2 gate,
enforcing on new data artifacts).

Run AFTER scripts/build_krishnagiri_rivers_basin.py; the builder writes through
...
Identity follows the catalogue: dataset = basins/<layer>, scope = krishnagiri-rivers.
...
BASIN = REPO / "public/data/basins/krishnagiri-rivers"
BUILD = "scripts/build_krishnagiri_rivers_basin.py"
...
TNGIS = reg("tngis-open-geoserver", "TNGIS open GeoServer (WFS): district, taluk, block and village panchayat boundaries, TN WRD sub-basins and reservoirs, micro-watershed atlas, all-water-bodies and all-tanks registers, mine leases, industry register matched to land parcels", "Tamil Nadu e-Governance Agency (TNGIS)",
            url="https://tngis.tn.gov.in/tngismaps/ows")
SIPCOT = reg("sipcot-gis-geoserver", "SIPCOT GIS (WFS): industrial complex outlines, Hosur phases I to IV, Shoolagiri and Bargur parks", "State Industries Promotion Corporation of Tamil Nadu (SIPCOT)",
             url="https://sipcotgis.tn.gov.in/")
OSM = reg("osm-overpass", "OpenStreetMap (Overpass API extract: named river courses)", "OpenStreetMap contributors")
...
CWC_RIVER = reg("nwic-nwdp-cwc-river-data", "CWC river discharge (manual daily) and surface water quality for Tamil Nadu, National Water Data Portal", "Central Water Commission, via the National Water Informatics Centre (NWIC)",
                url="https://nwdp.nwic.gov.in/")
...
ATLAS_PANCHAYAT_INPUTS = sorted(
    str(p.relative_to(REPO)) for fam in ("briefs", "census-2011") for p in (REPO / "public/data/atlas/tn/krishnagiri" / fam).glob("*.json")
)
RIVERS_IN = "public/data/basins/krishnagiri-rivers/rivers.geojson"
GW_IN = "public/data/atlas/tn/krishnagiri/groundwater-taluks.json"
SECTORS_IN = "pipeline-inputs/basins/krishnagiri-rivers/tnpcb-type-sectors.json"  # named in the note: internal_inputs takes catalogued artifacts only

# file (relative to the basin dir) -> (dataset, sources, internal_inputs, compact)
...
def stamp(fp: Path, dataset: str, sources: list[dict], internal_inputs: list[str], compact: bool, produced_at: str) -> None:
    ...
    prov: dict = {"sources": sources, "method": "derived", "produced_at": produced_at, "produced_by": BUILD}
    if internal_inputs:
        prov["internal_inputs"] = internal_inputs
    prov["note"] = NOTE
    doc = {"nvdm": "1.0", "dataset": dataset, "scope": {"kind": "basin", "id": "krishnagiri-rivers"}, "provenance": prov, **payload}
...
def main() -> None:
    ...
    # Station chart packs: CWC gauges (cwc-); the rest are TNPCB's sensor stations.
