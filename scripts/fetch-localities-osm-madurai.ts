/**
 * Fetch Madurai neighborhoods/suburbs/quarters from OpenStreetMap via the
 * Overpass API. Maps each locality to its MMC ward via point-in-polygon
 * and to its zone via madurai-ward-profiles. Output saved to
 * public/data/madurai-localities.json.
 *
 ...
 * "Tallakulam", "Anna Nagar", "Anaiyur" etc. should resolve to the
 * containing ward instead of falling through to ward-number-only match.
 *
 * Run: npx tsx scripts/fetch-localities-osm-madurai.ts
 */
...
// MMC bbox padded ~2 km on each side to catch peripheral suburbs OSM may
// have tagged just outside the corporation polygon (south,west,north,east).
const BBOX = "9.80,78.00,10.02,78.25";
...
SELECT DISTINCT ?item ?itemLabel ?taLabel ?coord WHERE {
  ...
  VALUES ?parent { wd:Q228405 wd:Q15184 }
  ...
  ?item wdt:P625 ?coord.
  OPTIONAL { ?item rdfs:label ?taLabel FILTER(LANG(?taLabel) = "ta"). }
...
// Madurai-specific note: OSM mapping in tier-2 cities is sparser than
// Chennai, so we widen the place taxonomy to include urban villages and
// hamlets - in Madurai, formerly-rural areas now inside MMC are commonly
// still tagged `place=village` rather than `place=suburb`. Mapping back
...
async function fetchOverpass(): Promise<OsmElement[]> {
  console.log("Fetching Madurai localities from OpenStreetMap (Overpass)...");
...
async function fetchWikidata(): Promise<WikidataBinding[]> {
  console.log("Fetching Madurai localities from Wikidata (SPARQL)...");
...
async function fetchLocalities(): Promise<void> {
  ...
  const wardGeoPath = join(process.cwd(), "public/geojson/madurai-wards-2022.geojson");
  const wardGeo = JSON.parse(readFileSync(wardGeoPath, "utf-8"));

  const profilesPath = join(process.cwd(), "public/data/madurai-ward-profiles.json");
  ...
  const stats: Record<string, Record<string, number>> = {
    osm: { kept: 0, noName: 0, noCoord: 0, outsideMmc: 0, dupe: 0 },
    wikidata: { kept: 0, noName: 0, noCoord: 0, outsideMmc: 0, dupe: 0 },
  };
  ...
  function consider(args: {
    ...
    if (!wardNum) { s.outsideMmc++; return false; }
    const profile = profileMap.get(wardNum);
    if (!profile) { s.outsideMmc++; return false; }
  ...
  const outPath = join(process.cwd(), "public/data/madurai-localities.json");
  writeFileSync(outPath, JSON.stringify(localities, null, 2));
  console.log(`\nWrote ${localities.length} localities to public/data/madurai-localities.json`);
  for (const [src, s] of Object.entries(stats)) {
    console.log(`  ${src}: kept=${s.kept}, dupe=${s.dupe}, noName=${s.noName}, noCoord=${s.noCoord}, outsideMMC=${s.outsideMmc}`);
  }
