...
REG_IDS = {
    "mpcb-water-quality-reports": ("MPCB annual Water Quality Status of Maharashtra reports (WQR series 2007-2024; 2019-20 never published)", "Maharashtra Pollution Control Board", "Maharashtra government publication, cited with attribution"),
    "praja-civic-issues-mumbai": ("Praja Foundation, 'Report on the Status of Civic Issues in Mumbai' (RTI-sourced ward tables; May 2025 edition = 2024 data)", "Praja Foundation (via OpenCity mirror)", "report (c) Praja, RTI-sourced tables; OpenCity mirror"),
    "wrd-flood-line-maps": ("Maharashtra WRD Flood Line Maps (legal red/blue-line sheets; 41 of 494 statewide cover the 6 MMR rivers)", "Maharashtra Water Resources Department", "Maharashtra government publication, cited with attribution"),
    "bmc-esr-annual": ("BMC Environment Status Report (annual)", "Brihanmumbai Municipal Corporation", "government publication, cited with attribution"),
    "bmc-rti-manuals": ("BMC Hydraulic Engineer RTI manuals (Dy HE Bhandup Complex + Water Supply Projects departments, Dec 2024)", "BMC Hydraulic Engineer's Department", "government publication, cited with attribution"),
    "bmc-climate-action-plan": ("BMC Climate Budget Report / Mumbai Climate Action Plan (via OpenCity)", "BMC Climate Action Cell (via OpenCity)", "open (per OpenCity dataset page)"),
    "cgwb-yearbook-maharashtra": ("CGWB Ground Water Year Book of Maharashtra (2024-25 canonical; 2022-23 stitched for multi-year depth) + GW Quality trend report", "CGWB Central Region, Nagpur", "GoI publication, cited with attribution"),
    "mahagr-wrd-resolutions": ("Maharashtra WRD Government Resolutions (via the orgpedia mahGRs mirror)", "Maharashtra WRD (via the orgpedia mahGRs mirror)", "government resolutions, mirrored openly"),
    "mpcb-stp-inventory": ("MPCB per-STP inventory (STP_information_on_website_publication)", "Maharashtra Pollution Control Board", "government publication, cited with attribution"),
    "wrd-pravah-dam-feed": ("Maharashtra WRD Pravah dam-safety daily bulletin (all-Maharashtra live-storage PDF, 139 dams)", "Maharashtra WRD (Pravah dam-safety portal)", "Maharashtra government public feed, no stated licence - cited with attribution"),
    "bmc-dm-floodspots": ("BMC Disaster Management flood-spot register (floodSpot/loadAll API)", "BMC Disaster Management (dmwebtwo.mcgm.gov.in)", "BMC government portal API, no stated licence - cited with attribution"),
...
PROVENANCE: dict[str, dict] = {
    ...
    "data-root/industrial-sources": {
        ...
        "sources": [
            ...
            reg("praja-civic-issues-mumbai", as_of="2024"),
            dict(OSM_SOURCE, title="OpenStreetMap plant locations (eastern-corridor STPs)"),
        ],
        "note": (
    ...
    "data-root/cgwb-stations": {
        ...
        "sources": [reg("cgwb-yearbook-maharashtra", as_of="2025", retrieved="2026-07-26")],
        "note": (
    ...
    "data-root/ward-water-praja": {
        "method": "pdf-extract",
        "produced_by": "manual",
        "sources": [reg("praja-civic-issues-mumbai", as_of="2024", retrieved="2026-07-21")],
        "note": (
    ...
    "geojson-layers/coastal-zones": {
        ...
        "internal_inputs": [],
        "sources": [
            dict(OSM_SOURCE, title="OpenStreetMap coastline (Overpass extract, seaward face)", role="input"),
            MSMP_2017,
            NCCR_2018,
        ],
        "note": (
    ...
    "geojson-layers/corporations-2024": {
        ...
        "sources": [dict(OSM_SOURCE, title="OpenStreetMap administrative boundaries (Overpass extract)")],
        "note": (
    ...
    "geojson-layers/drainage": {
        ...
        "sources": [dict(OSM_SOURCE, title="OpenStreetMap drain/nalla network (Overpass extract)")],
        "note": (
...
LEGACY_DATES = ("generated_at", "compiled_at", "computed_at", "updated", "last_updated", "fetched_at", "retrieved", "_fetched")
SKIP = {
