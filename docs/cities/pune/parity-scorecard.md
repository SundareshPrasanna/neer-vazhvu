...
## Medium - present and usable, thinner than Chennai (7)
...
| Drainage / storm-water geometry | 10,308 features | **3,075 nalla segments, 1,014 km** | Ships on `/pune/flood-risk`. Thinner in count and **unnamed**: not one of the 3,075 carries a name, so Ambil Odha - the nalla the 2019 flash flood came down - cannot be identified in it. PMC also publishes 141,341 buried pipe segments, deliberately left out as too dense to draw and a maintenance rather than a flooding question. The layer does check itself: computed lengths agree with PMC's own length column to 0.4%. |
...
| Feature | Chennai | Pune |
|---|---|---|
| **Flood-risk hazard layers** | 5 hazard geojson layers (2015 + 2020 hotspots, hazard zones, inundation depth, return periods) | **0, and the page ships anyway.** `/pune/flood-risk` now renders on the NARRATIVE variant, which needs no hazard polygons: the event register plus PMC's 1,014 km nalla network. What is still missing is only the hazard layer itself. Maharashtra WRD publishes Pune's red (100-year) and blue (25-year) flood lines as **518 scanned PDF map sheets and zero vector files.** `pdftotext` extracts no characters from the Mutha sheets. The event register is solid - 12 Jul 1961 Panshet, the 2019 Ambil Odha flash flood, 25 Jul and 4 Aug 2024, 21 Aug 2025 - but there is nothing to draw. Digitising 518 raster sheets is a project, not a fetch. |
...
**It no longer withholds the page, though, and that distinction was worth correcting.** The route was
off on the reasoning that the hazard layer does not exist - true, but an argument about the
*interactive* variant. The narrative stack needs no hazard polygons, and Pune was holding 1,014 km of
nalla geometry that rendered nowhere. The absence now ships as the first data gap **on** the page,
which is what this platform is supposed to do with a gap.
