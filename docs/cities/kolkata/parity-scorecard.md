...
| Climate-risk sub-basins | 6 sub-basins, 4 components | none | **Correction to an earlier entry here:** Chennai's sub-basins are NOT our own DEM work - they come from WWF/HydroSHEDS HydroBASINS level 12, a pre-computed global product, and its methodology note already ships a flat-coast caveat for the canal-cross-linked Pulicat edge. So "too flat" was the wrong reason to withhold; the Chennai recipe would transfer with the same `boundary_quality` tagging. Genuinely buildable, not a method refusal. |
| Per-water-body catchment atlas | Clickable area of influence per lake, + rooftop harvest | none, **and the page now says why** | This one IS a terrain limit. The atlas traces D8 flow accumulation over a 30 m bare-earth DEM, which needs a gradient: Kolkata has **11 m of relief across 40 km**, against Chennai 43 m, Madurai 94 m, Bengaluru 194 m, Mumbai 338 m - the four cities where it ships. With 5,526 water bodies each claiming a catchment, and runoff actually moving through a combined sewer network rather than over the ground, the output would look authoritative and be wrong. Surfaced as an explained absence in the view-mode toggle rather than a hidden button. |
| Rooftop rainwater-harvest estimate | Per-lake, from the catchment | none | An enrichment of the catchment polygon, so it falls with it. The inputs exist (Overture footprints + IMD normal 1,641 mm); a **ward-level** version is buildable against a different boundary, and in Kolkata it doubles as a drainage intervention - every litre held on a roof is a litre not hitting a 6 mm/h sewer. |
