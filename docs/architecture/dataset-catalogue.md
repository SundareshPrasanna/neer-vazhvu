# Dataset catalogue (file artifacts)

Generated by `scripts/build_dataset_catalogue.py` against `public/data`, `public/geojson`.
Machine-readable detail: `dataset-catalogue.json` (per-file schema fingerprints, consumers, producers, Headwaters joins).

**473 files · 246.5 MB · 139 logical datasets · 58 Headwaters sources. Accountability: 250 watched · 105 allowlisted · 118 unaccounted.**

## Families

| family | files | MB | scopes | watched | allowlisted |
|---|--:|--:|--:|--:|--:|
| cascade | 48 | 127.9 | 4 | 48 | 0 |
| geojson-layers | 94 | 63.6 | 6 | 17 | 64 |
| basins | 92 | 34.2 | 4 | 4 | 0 |
| data-root | 111 | 19.5 | 5 | 53 | 41 |
| rich-bodies | 127 | 1.4 | 2 | 127 | 0 |
| ingres | 1 | 0.0 | 1 | 1 | 0 |

## Standardization signal

- Logical datasets present in **2+ cities**: 53
- Logical datasets in **exactly 1 city** (bespoke): 44
- Multi-city datasets whose per-city files have **divergent schemas**: 37

### Multi-city datasets with schema drift (fix-first list for NVDM)

| family | dataset | cities | schema variants |
|---|---|---|--:|
| data-root | river-quality | bangalore, chennai, delhi, madurai, mumbai | 5 |
| data-root | supply-overview | bangalore, chennai, delhi, madurai, mumbai | 5 |
| geojson-layers | water-bodies-current | bangalore, chennai, delhi, madurai, mumbai | 5 |
| data-root | cgwb-stations | bangalore, delhi, madurai, mumbai | 4 |
| data-root | gwr-blocks | bangalore, chennai, delhi, madurai | 4 |
| data-root | industrial-sources | bangalore, chennai, delhi, madurai, mumbai | 4 |
| data-root | ward-profiles | bangalore, chennai, delhi, madurai | 4 |
| data-root | ward-risk | bangalore, delhi, madurai, mumbai | 4 |
| geojson-layers | rivers | bangalore, chennai, delhi, madurai, mumbai | 4 |
| cascade | cascade-lakes | bangalore, chennai, madurai, mumbai | 3 |
| data-root | commitments | bangalore, chennai, delhi, madurai, mumbai | 3 |
| data-root | flood-hotspots | bangalore, delhi, mumbai | 3 |
| data-root | localities | bangalore, chennai, delhi, madurai | 3 |
| data-root | restoration-priority | bangalore, chennai, delhi, madurai, mumbai | 3 |
| data-root | restoration-projects | chennai, delhi, madurai, mumbai | 3 |
| data-root | water-bodies-flagship | delhi, madurai, mumbai | 3 |
| data-root | water-bodies-lost | bangalore, delhi, madurai, mumbai | 3 |
| geojson-layers | drainage | chennai, delhi, mumbai | 3 |
| geojson-layers | wards-2022 | chennai, delhi, madurai | 3 |
| geojson-layers | water-bodies-lost | chennai, madurai, mumbai | 3 |
| cascade | cascade-sensitivity | chennai, madurai | 2 |
| cascade | cascade-stats | bangalore, chennai, madurai, mumbai | 2 |
| cascade | cascades-documented | bangalore, chennai, madurai | 2 |
| cascade | cascades-health | bangalore, chennai, madurai | 2 |
| cascade | catchment-quality | bangalore, chennai, madurai, mumbai | 2 |
| data-root | allocations | bangalore, chennai, delhi, madurai, mumbai | 2 |
| data-root | facts | bangalore, delhi, madurai, mumbai | 2 |
| data-root | gee-phase1-water-body-targets | chennai, madurai | 2 |
| data-root | gw-stations | chennai, madurai | 2 |
| data-root | imd-rainfall-monthly | bangalore, chennai, delhi, madurai, mumbai | 2 |
| data-root | rainfall-recent | bangalore, chennai, delhi, madurai, mumbai | 2 |
| data-root | river-events | bangalore, delhi, madurai | 2 |
| data-root | ward-representatives | chennai, delhi | 2 |
| geojson-layers | buffer-1000m | bangalore, chennai | 2 |
| geojson-layers | polygon | bangalore, chennai | 2 |
| geojson-layers | water-bodies-census | bangalore, delhi | 2 |
| rich-bodies | imagery-manifest | bangalore, chennai | 2 |

## Hygiene

- **Orphans** (no consumer, no producer script, no Headwaters source): 1
  - `public/geojson/delhi-microwatersheds.geojson` (7.1 MB)
- **Unaccounted files** (no registry lineage AND no allowlist reason): 118 of 473
- **Chennai legacy unprefixed names**: 6
- **Unknown scope** (city not derivable from path): 0

## Largest files

| MB | path | consumers |
|--:|---|--:|
| 21.1 | `public/data/cascade/bangalore-catchment-downstream.json` | 1 |
| 19.1 | `public/data/cascade/bangalore-catchment-streams.json` | 1 |
| 12.0 | `public/data/cascade/madurai-cascade-catchments.geojson` | 1 |
| 11.8 | `public/data/cascade/madurai-catchment-downstream.json` | 1 |
| 11.7 | `public/data/cascade/bangalore-catchment-basin.json` | 1 |
| 8.8 | `public/data/cascade/bangalore-cascade-catchments.geojson` | 1 |
| 8.8 | `public/data/cascade/madurai-catchment-streams.json` | 1 |
| 7.4 | `public/data/cascade/chennai-catchment-streams.json` | 1 |
| 7.1 | `public/geojson/delhi-microwatersheds.geojson` | 0 |
| 6.7 | `public/geojson/delhi-wards-2022.geojson` | 0 |
| 6.5 | `public/geojson/bangalore-water-bodies-current.geojson` | 3 |
| 5.9 | `public/data/elevation-bands-mumbai.geojson` | 1 |
| 5.9 | `public/data/cascade/chennai-cascade-catchments.geojson` | 1 |
| 4.4 | `public/data/cascade/chennai-catchment-basin.json` | 1 |
| 4.3 | `public/geojson/chennai-drainage.geojson` | 1 |
