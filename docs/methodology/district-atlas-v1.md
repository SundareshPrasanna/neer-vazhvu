...
Karnataka (Kolar, preview-gated) runs on the LGD adapter with four state differences, each a reviewed field in its plan: the DCHB release prints each village's Panchayat name without a code (`censusGramPanchayatColumns: "names-without-codes"`, so composition still comes from the LGD register); DataMeet's Karnataka file carries each polygon's 2001 district and village codes and a semicolon 2011-to-2001 mapping instead of a CEN_2001 key (`crosswalkFormat: "village-code-mapping"`); the LGD export lists only one or two villages per Kolar Panchayat (286 of 1,825), so their union is not the Panchayat's extent and the polygons are withheld (`polygons: "withheld"`: the centroid places the marker, no outline or area is shown, the boundary capability is not evidenced); and the First Census of Water Bodies Karnataka return carries a local serial rather than the Census 2011 village code, so water bodies stay a named gap. The CPCB polluted-stretches input covers Tamil Nadu and Maharashtra only, so the family is not assessed for Karnataka rather than served as "none listed".
