...
- **My Ward** - Per-ward report aggregating every layer above with comparison + uplift planner (Chennai, Madurai, Bengaluru, Delhi, Gurugram). Deliberately withheld where the geometry is not public: Mumbai until the ward build lands, Hyderabad because the 300-ward delimitation gazetted 25 Dec 2025 has no published boundaries, and Kolkata because the only public ward geometry covers 141 of 144 wards - the three missing ones are 9.2% of the city. Every omission and its reason is in [docs/architecture/exemptions.md](docs/architecture/exemptions.md)
