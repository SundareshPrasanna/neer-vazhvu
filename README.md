...
- **Coast & shoreline change** (Chennai only today) - Erosion/accretion along the 86 km Chennai-Ennore-Pulicat coast. A "Study zones" layer surfaces the published per-zone rates + port hotspots from Anagha, Singh & Frappart (2026, *Environmental Challenges*, 1990-2024) over the OSM coastline; an "Our transects" layer is our own MNDWI/DSAS reproduction (Landsat 5/7/8 + Sentinel-2 via Earth Engine, 972 transects, **1990-2026** - extending the study's window with current Sentinel-2) that independently corroborates the study's pattern and adds a per-transect movement-over-time chart + acceleration check (≈66% of the eroding coast is now eroding faster than before)
