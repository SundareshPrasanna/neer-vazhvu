#!/usr/bin/env python3
"""Build public/data/hyderabad-cgwb-stations.json from the India-WRIS API.

Hyderabad's groundwater layer is point observations, not a surface. The CGWB
network here is far sparser than Delhi's (~50 stations across the metro core
against Delhi's 237), and Telangana's own Ground Water Department runs a much
denser village-level piezometer network that would be the better source if its
live feed can be reached. Until then these are the honest points, and
`groundwaterViews.depth` stays FALSE in the city config: ~50 stations cannot
paint a continuous surface across 300 wards.

TWO TRAPS specific to Telangana, both verified 2026-07-26 and both silent:

  1. WRIS carries a partly PRE-2016 district set. Telangana reorganised from
     10 districts to 33 in Oct 2016 and WRIS did not fully follow, so
     `Ranga Reddy`, `Medak`, `Siddipet` and `Vikarabad` return data while
     `Medchal-Malkajgiri`, `Sangareddy` and `Yadadri Bhuvanagiri` return
     statusCode 500 / "No data found". Enumerate spellings empirically; do
     NOT derive them from the current administrative list.
  2. A too-narrow date window is indistinguishable from "no stations".
     `Ranga Reddy` returns nothing for 2024-2025 but data for 2015-2026.
     Probe wide, then narrow.

Unlike Delhi's network - which stopped 2025-09-20 - Hyderabad's is LIVE, with
telemetric readings to 2026-06-04.
...
OUT = REPO / "public/data/hyderabad-cgwb-stations.json"
...
# Spellings VERIFIED to return rows (2026-07-26). See trap 1 in the docstring:
# the post-2016 district names fail, so this is deliberately the legacy set.
# Hyderabad + Ranga Reddy are the metro core; Medak, Siddipet and Vikarabad
# cover the ring the Core Urban Region absorbed.
DISTRICTS = [
    "Hyderabad",
    "Ranga Reddy",
    "Medak",
...
# These were probed and return statusCode 500 / "No data found" for ANY window.
# Kept as a record so the next person does not re-derive it.
DISTRICTS_NOT_IN_WRIS = [
    "Medchal-Malkajgiri",
    "Sangareddy",
    "Yadadri Bhuvanagiri",
]
...
# Populated empirically from the first full run - see the sensor audit printed
# at the end. Left EMPTY rather than copied from Delhi: a suspect-sensor list is
# a per-network finding, and inheriting Delhi's would be meaningless here.
SUSPECT: dict[str, str] = {}
# Physically defensible envelope for Deccan hard-rock depth-to-water. Hyderabad
# sits on granite/gneiss where the water table is shallow but bore wells chase
# fractures deep; the Telangana GWD has recorded piezometer readings past 70 m
...
def main() -> None:
    ...
    doc = {
        ...
        # Consumed by CgwbStationPanel. These are telemetric/manual observation
        # wells, not a Year Book transcription of dug wells, so the provenance
        # and cadence lines are overridden rather than inheriting the Tamil Nadu
        # defaults.
        ...
        "coverage": {
            "period": f"{min(YEARS)} to {max(YEARS) - 1}",
        ...
        "_districts_not_in_wris": DISTRICTS_NOT_IN_WRIS,
