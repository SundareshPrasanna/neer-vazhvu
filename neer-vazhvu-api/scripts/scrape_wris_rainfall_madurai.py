#!/usr/bin/env python3
"""
Daily WRIS rainfall ingest for the Madurai / Vaigai system. Fans out
across Madurai + Theni + Dindigul + Virudhunagar districts to capture
the full Vaigai catchment and adjacent reservoirs.

Probed coverage (CWC, Apr-May 2026):
    Madurai       : VADAMADURAI
    Theni         : Vaigai Reservoir (the dam itself!)
    Dindigul      : KODAGANAR DAM
    Virudhunagar  : IRRUKKANKUDI

Daily total = MAX of the IPC accumulator (midnight-reset cumulative counter).

Usage:
    cd neer-vazhvu-api
    python scripts/scrape_wris_rainfall_madurai.py
    # window override:
...
async def main() -> int:
    ...
    days = _env_int("MADURAI_WRIS_RAIN_DAYS", 90)
    end = date.today()
    start = end - timedelta(days=days)

    print(
        f"Fetching WRIS Vaigai-system rainfall records {start} -> {end} "
    ...
    for district in VAIGAI_SYSTEM_DISTRICTS:
        try:
            d_records = await fetch_wris_rainfall(
                start_date=start,
                end_date=end,
                district=district,
                agencies=["CWC", "IMD"],
